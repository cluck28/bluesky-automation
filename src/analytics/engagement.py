...
def get_likes_df(likes: list, follows: list, followers: list) -> DataFrame:
    ...
    merged = likes_df.merge(follows_df[["handle", "follow_index"]], on="handle", how="left")
    ...
    merged = merged.merge(followers_df[["handle", "follow_index"]], on="handle", how="left")
...
def get_engagement_score(likes_df: DataFrame, followers: int) -> int:
    ...
    return round((filtered_df[filtered_df["follower"]]["handle"].nunique() / followers)*100, 0)
